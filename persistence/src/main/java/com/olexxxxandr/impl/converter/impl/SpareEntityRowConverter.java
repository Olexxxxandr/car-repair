...
public class SpareEntityRowConverter implements EntityRowConverter<SpareEntity> {
        ...
        try {
            ...
            WorkroomEntity workroomEntity =
                    DaoFactory.getInstance()
                            .getWorkroomDao()
                            .findOneById(
                                    UUID.fromString(resultSet.getString("workroom_id")), connection)
                            .orElseThrow(
                                    () ->
            ...
            var spareEntity =
                    SpareEntity.builder()
                            ...
                            .price(
                                    new Money(
                                            resultSet.getInt("price_whole_part"),
                                            resultSet.getInt("price_decimal_part")))
            ...
            String message =
                    "Не вдалось отримати ResultSet в %s. Детально: %s"
                            .formatted(
                                    SpareEntityRowConverter.class.getSimpleName(), e.getMessage());
    ...
    private static void setExtraFieldOnManyToMany(ResultSet resultSet, SpareEntity spareEntity)
            throws SQLException {
        ResultSetMetaData metaData = resultSet.getMetaData();
        int columnCount = metaData.getColumnCount();

        for (int i = 1; i <= columnCount; i++) {
            if (metaData.getColumnName(i).equals("extra_quantity")) {
                spareEntity.setExtraFieldQuantity(resultSet.getString("extra_quantity"));
                break;
            }
        }
    }
