        ...
        try {
            ...
            EmployeeEntity employeeEntity =
                    DaoFactory.getInstance()
                            .getEmployeeDao()
                            .findOneById(
                                    UUID.fromString(resultSet.getString("employee_id")), connection)
                            .orElseThrow(
                                    () ->
                                            new EntityNotFoundException(
                                                    "Не вдалось отримати об'єкт сутності"
                                                        ...
                                                        + " базі даних."));
            var payrollEntity =
                    PayrollEntity.builder()
                            .id(UUID.fromString(resultSet.getString("id")))
                            .employeeEntity(employeeEntity)
                            .periodType(
                                    PayrollEntity.PeriodType.valueOf(
                                            resultSet.getString("period_type")))
                            .hourCount(resultSet.getInt("hour_count"))
                            .salary(
                                    new Money(
                                            resultSet.getInt("salary_whole_part"),
                                            resultSet.getInt("salary_decimal_part")))
            ...
            String message =
                    "Не вдалось отримати ResultSet в %s. Детально: %s"
                            .formatted(
                                    PayrollEntityRowConverter.class.getSimpleName(),
                                    e.getMessage());
